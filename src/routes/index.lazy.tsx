...
import heroPoster from "@/assets/posters/hero-poster.jpg";
import {
  HERO_BASE,
  HERO_DURATION,
  HERO_STAGGER,
...
function HomePage() {
  ...
  // Play the video only after the splash overlay signals it is done.
  // The 600 ms delay covers the 500 ms exit fade so the video starts
  // precisely when the screen is fully clear.
  useEffect(() => {
    if (!splashDone) return;
    const timeout = setTimeout(() => {
      if (videoRef.current) {
        videoRef.current.currentTime = 0;
        videoRef.current.play().catch((err) => console.log("Autoplay evitado:", err));
      }
    }, 600);
  ...
  return (
    <SiteShell headerVariant="dark">
      ...
      <section className="relative h-screen overflow-hidden bg-black text-white">
        ...
        {/* Z-10 — Overlay preto semi-transparente */}
